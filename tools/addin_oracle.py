...
answers the JavaScript add-in core (addin/core/plan.js) is held to.
    ...
    python3 tools/addin_oracle.py --write    # rewrite the golden
...
def main(argv: list[str]) -> int:
    want = render(golden())
    have = GOLDEN.read_text(encoding="utf-8") if GOLDEN.is_file() else ""
    if have == want:
        print(f"{GOLDEN.name}: in sync with the converter's plan_table()")
        return 0
    if "--write" in argv:
        GOLDEN.write_text(want, encoding="utf-8")
        print(f"{GOLDEN.name}: rewritten -- read the diff before committing it")
        return 0
    print(f"{GOLDEN.relative_to(ROOT)} no longer matches the converter's "
          f"plan_table(); run tools/addin_oracle.py --write and read the diff")
    return 1
